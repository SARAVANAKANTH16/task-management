const taskContainer= document.querySelector(".task__container");
const globalStore=[];
console.log(taskContainer);
//black tick is for multiple html code so that we should it inside {}
const generateNewCard= (taskData) => `
    <div class="col-sm-12 col-md-6 col-lg-4" id=${taskData.id}>
          <div class="card">
            <div class="card-header d-flex justify-content-end gap-2">
              <button type="button" class="btn btn-outline-success"><i class="fas fa-pencil-alt"></i></button>
              <button type="button" class="btn btn-outline-danger"><i class="fas fa-trash-alt"></i></button>
            </div>
            <div class="card-body">
              <img class="card-img-top" src=${taskData.imageUrl} alt="...">
               
               <h5 class="card-title mt-3 fw-bolder text-primary">${taskData.taskTitle}</h5>
    <p class="card-text">${taskData.taskDescription}</p>
    <a href="#" class="btn btn-primary">${taskData.taskType}</a>
  </div>
  `;

const loadInitialCardData = () => {
  //isssues
  //localstorage to get tasky card data
  const getCardData = localStorage.getItem("tasky");

  //covert to normal object
  const {cards} = JSON.parse(getCardData);
  //converting arr(obj) into noraml obj
  //loop over those array of task object to create Html card ,
  //inject it to Dom
  //destructing
  cards.map((cardObject)=> {
    taskContainer.insertAdjacentElement("beforeend",generateNewCard(cardObject));
    globalStore.push(cardObject);

  })
  //update our global store

};


const saveChanges = () => {
    const taskData = {
        id: `${Date.now()}`,     //keyvalue para it used for to generate a unique id we use this
        imageUrl: document.getElementById("imageurl").value,
        taskTitle:document.getElementById("tasktitle").value,
        taskType: document.getElementById("tasktype").value,
        taskDescription: document.getElementById("taskdescription").value
    };
    
  taskContainer.insertAdjacentHTML("beforeend",generateNewCard(taskData));

globalStore.push(taskData);
localStorage.setItem("tasky",JSON.stringify({cards:globalStore}));//id:tasky id is for unique
};

//when changes push it is in object of object format but it should be arr object format
//so that we use stringfy 

//${xyz} it will change in future so that we use $ so it is static
//ISSUES
//page refreshers causes the data to get deleted
//local storage->AcCessing application via local storage
//Interface--> Interface means middle man 
//Features- Delete , edit , open the card
